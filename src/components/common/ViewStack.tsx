...
export function ViewStack<TState = void, TResult = void>({
  isOpen,
  onClose,
  initialView,
  className,
  onComplete,
}: ViewStackProps<TState, TResult>) {
  ...
  useEffect(() => {
    ...
    if (executions.length > 0) {
      ...
      for (const [, command] of executions) {
        void command.execute().catch((err: unknown) => {
          console.error('[ViewStack] Command execute failed:', err);
        });
      }
    }
    ...
    if (undos.length > 0) {
      ...
      for (const [, command] of undos) {
        void command.undo().catch((err: unknown) => {
          console.error('[ViewStack] Command undo failed:', err);
        });
      }
    }
  }, [viewStack]);
  ...
  const pushView = useCallback(
    <TState = void, TResult = void>(view: View<TState, TResult>) => {
